...
def batch_and_forward(model, stack, device):
    batched = torch.stack(stack)
...
def calc_val_far(confusion_matrix):
    far, val = confusion_matrix[:, 1] / np.sum(confusion_matrix, 1)
    return val, far

def threshold_from_far(far, embeddings, labels, threshold_range=(0, 4, 0.01), device = None):
    logger = logging.getLogger("ThresholdFromFAR")
    threshold = np.arange(*threshold_range)
    ...
    def val_far(pred, actual):
        ...
        return val, far
    def pred_actual(threshold, embeddings=embeddings, labels=labels, device=device):
        pred = []
        actual = []
    ...
    for idx in range(len(threshold)):
        pred, actual = pred_actual(threshold[idx])
        val, far_ = val_far(pred, actual)
        logger.info(f"Current threshold: {threshold[idx]:.3f}, val: {val:.3f}, far: {far_:.3f}")
        if far_ > far:
            approx = threshold[idx] - (far_ - far) / (far_ - pred_far) * (threshold[idx] - threshold[idx - 1])
            return approx
        pred_far = far_
...
def evaluate(embeddings, actual_issame, nrof_folds=10, distance_metric=0, subtract_mean=False, far_target=1e-3):
