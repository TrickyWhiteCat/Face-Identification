...
import sys
sys.path.append(os.path.join(os.path.abspath(os.path.pardir),"etos-mtcnn-master/prepare_data"))
print(sys.path)
...
def train(net_factory, prefix, end_epoch, base_dir,
    ...
    """
    wandb.login( 
    key = "c4bf36ba0f80f31563b44695b0b659101303bf8f",
    )   
    wandb.init(project="MTCNN_ONet", name="ONet")
    net = prefix.split('/')[-1]
    #label file
    ...
    #RNet use 3 tfrecords to get data    
        ...
        if net == 'RNet':
            landmark_dir = os.path.join(r'C:/Users/data_/imglists_noLM/RNet','landmark_landmark.tfrecord_shuffle')
        else:
    ...
    try:
        for step in range(MAX_STEP):
            ...
            if (step+1) % display == 0:
                ...
                wandb.log({'accuracy':acc,'cls_loss':cls_loss,'bbox_loss':bbox_loss,'landmark_loss':landmark_loss,'Total Loss':total_loss,'lr':lr})
